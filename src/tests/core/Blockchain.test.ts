...
describe('The Blockchain', ()=>{
	...
	it('creates a blockchain with a list of blocks with invalid genesis block is not allowed', ()=>{
		...
		expect(()=> BlockChain.create([noGenesisBlock], 0)).toThrow()
	});
	...
	it('synchronizes with another blockchain by taking the blockchain with more valid blocks given', ()=>{
		...
		const blockChain = BlockChain.create([genesisBlock],0);
		const anotherBlockChain = BlockChain.create([genesisBlock, Block.createFrom({timestamp:'0', previousBlockHash:genesisBlock.hash, transactions:'irrelevant-data', nonce:0} )],0);
	...
	it('does not synchronize with another blockchain that has less blocks', ()=>{
		const genesisBlock = Block.createGenesisFrom({timestamp:'0', transactions:'irrelevant-data', nonce:0});
		const blockChain = BlockChain.create([genesisBlock, Block.createFrom({timestamp:'0', previousBlockHash:genesisBlock.hash, transactions:'irrelevant-data', nonce:0})],0);
		const anotherBlockChain = BlockChain.create([genesisBlock],0);
	...
	it('does not synchronize with another blockchain that has invalid genesis block', ()=>{
		...
		const blockChain = BlockChain.create([genesisBlock],0);
		const anotherBlockChain = BlockChain.create([invalidGenesisBlock, Block.createFrom({timestamp:'0', previousBlockHash:invalidGenesisBlock.hash, transactions:'irrelevant-data', nonce:0})],0);
	...
	it('does not synchronize with another blockchain that has a corrupted block', ()=>{
		...
		const blockChain = BlockChain.create([genesisBlock, secondBlock],0);
		...
		const anotherBlockChain = BlockChain.create([genesisBlock, corruptedBlock, thirdBlock],0);
	...
	it('does not synchronize with another blockchain that has corrupted previous hash in some block', ()=>{
		...
		const thirdBlock = Block.createFrom({timestamp:'0', previousBlockHash:secondBlock.hash, transactions:'irrelevant-data', nonce:0});
		const anotherBlockChain = BlockChain.create([genesisBlock, blockWithCorruptedPreviousHash, thirdBlock], 0);
	...
	context('when difficulty threshold is one', ()=>{
		it('concatenates a new mined block that includes previous hash', ()=>{
			...
			const blockChain = BlockChain.createFrom('0', difficultyThreshold);
			const candidateBlock = Block.createFrom({timestamp:'1', previousBlockHash:blockChain.getLastBlock().hash, transactions:'irrelevant-data', nonce:0} )
			const minedBlock = ProofOfWorkService.create(difficultyThreshold).mineBlock(candidateBlock)

			expect(blockChain.concatBlock(minedBlock).getLastBlock().isEquals(minedBlock)).toBeTruthy()
		});
		...
		it('does not allow concatenate a candidate block', ()=>{
			...
			expect(()=>blockChain.concatBlock(candidateBlock)).toThrow()
		});
	});
